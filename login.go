...
func (wechat *WeChat) beginLoginFlow() error {
	...
	log.Debug(`请稍候，准备登录参数 ...`)

	cached, err := wechat.cachedInfo()

	if err == nil {

		log.Debug(`尝试使用 session 信息登陆...`)
...
func (wechat *WeChat) cachedInfo() (map[string]interface{}, error) {
	...
	if err != nil || len(baseInfo) != 3 {
		return nil, errors.New(`缓存基本信息无效...`)
	}

	cookies, err := wechat.cachedCookies()
	if err != nil || len(cookies) == 0 {
		return nil, errors.New(`cookies 缓存无效...`)
	}
...
func (wechat *WeChat) keepAlive() {
	go func() {
		...
		if err != nil {
			...
			log.Warnf(`将在%d分钟后重试登录`, retryTimes)
			<-triggerAfter
			wechat.retryTimes++
			wechat.keepAlive()
			return
